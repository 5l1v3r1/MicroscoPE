...
void dosHead(PIMAGE_DOS_HEADER dHeader) {
...
void ntHead(PIMAGE_NT_HEADERS pHeader) {
    SetConsoleTextAttribute(console, 15);
    std::cout << "-----NT Header-----" << std::endl;
    SetConsoleTextAttribute(console, 13);
    std::cout << "\tSignature: 0x" << std::hex << pHeader->Signature << std::endl;
}
void optHead(PIMAGE_DOS_HEADER dHeader, PIMAGE_NT_HEADERS pHeader) {
...
void fHead(PIMAGE_NT_HEADERS pHeader, PIMAGE_SECTION_HEADER sHeader) {
...
void sectHead(PIMAGE_NT_HEADERS pHeader, PIMAGE_SECTION_HEADER sHeader) {
...
int main(int argc, LPCSTR argv[]) {
    ...
    PIMAGE_DOS_HEADER dHeader = (PIMAGE_DOS_HEADER)lpFileBase;

    PIMAGE_NT_HEADERS pHeader = (PIMAGE_NT_HEADERS)((u_char*)dHeader + dHeader->e_lfanew);
    PIMAGE_SECTION_HEADER sHeader = IMAGE_FIRST_SECTION(pHeader);
	...
	if (argc == 1){
       ...
       OPTIONAL HEADER
       */
        optHead(dHeader,pHeader);
        /*
        FILE HEADER
        */
        fHead(pHeader,sHeader);
        /*
        Section HEADER
        */
        sectHead(pHeader, sHeader);
        other();
